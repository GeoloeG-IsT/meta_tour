-- Complete SoulTrip Database Setup
-- Run this script in your Supabase SQL Editor to set up the database with role fixes
...
-- Drop existing tour policies if they exist
DROP POLICY IF EXISTS "Anyone can view published tours" ON tours;
DROP POLICY IF EXISTS "Organizers can view their own tours" ON tours;
DROP POLICY IF EXISTS "Organizers can create tours" ON tours;
DROP POLICY IF EXISTS "Organizers can update their own tours" ON tours;
DROP POLICY IF EXISTS "Organizers can delete their own tours" ON tours;
...
DROP POLICY IF EXISTS "Participants can view their own bookings" ON bookings;
DROP POLICY IF EXISTS "Organizers can view bookings for their tours" ON bookings;
DROP POLICY IF EXISTS "Participants can create bookings" ON bookings;
DROP POLICY IF EXISTS "Participants can update their own bookings" ON bookings;
DROP POLICY IF EXISTS "Organizers can update bookings for their tours" ON bookings;
...
DROP POLICY IF EXISTS "Participants can view payments for their bookings" ON payments;
DROP POLICY IF EXISTS "Organizers can view payments for their tour bookings" ON payments;
...
-- Create an improved function to handle new user signup with proper role handling
...
DROP TRIGGER IF EXISTS validate_role_trigger ON users;
...
CREATE INDEX IF NOT EXISTS idx_bookings_participant_id ON bookings(participant_id);
CREATE INDEX IF NOT EXISTS idx_bookings_status ON bookings(status);
CREATE INDEX IF NOT EXISTS idx_payments_booking_id ON payments(booking_id);

-- Success message
SELECT 'SoulTrip database setup completed successfully! 🎉' as message;